...
```{r}
left_enrollment = left_join(by = c("id", "title", "name", "department"), x = sep_old, y = sep_new)

left_enrollment <- left_enrollment %>%
  mutate(enrollment_difference = u_grad.x - u_grad.y) %>%
  arrange(enrollment_difference)

View(left_enrollment)
```
